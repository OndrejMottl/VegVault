...
Abiotic data is aimed to procide the information about all relevant abiotic information affecting vegetation disctribution and its traits.

Abiotic data is linked to the structure of the **VegVault** Database by `gridpoints`, which are articfically cretaed pointed to *reasonably* cover the resolution of both modern and past data for vegetation and abiotic data.
...
There are currently abiotic from [CHELSA](https://chelsa-climate.org/) and [CHELSA-TRACE21](https://chelsa-climate.org/chelsa-trace21k/) and [WoSIS](https://www.isric.org/explore/wosis). CHELSA and CHELSA-TRACE21 is build on the same same structure of variables (visit the websites for more info).
...
Abitic data is simply linked to `Samples`.

```{r}
#| label: AbioticData scheme structure
knitr::include_graphics(
  here::here(
    "R/03_Supplementary_analyses/DB_scheme_visualisation/AbioticData.png"
  )
)
```
...
data_bio1_example %>%
  ...
  ggplot2::labs(
    title = "Example Mean Annual temprateture at diferent age slices (cal yr BP)",
    x = "Longtitude",
    y = "Latitude",
    col = "Temperature (°C)",
    fill = "Temperature (°C)"
...
Note that the spatial resolution is higer for modern climate data than for the past. this is to reduce the size of the past climate data.
