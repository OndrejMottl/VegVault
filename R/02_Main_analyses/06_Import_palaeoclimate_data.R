...
data_palaeoclimate_nested <-
  ...
  tidyr::nest(
    data_climate = c(
      time_id, bio1, bio4, bio6, bio12, bio15, bio18, bio19
    )
  )
...
palaeoclimate_samples_raw <-
  ...
  dplyr::mutate(
    sample_name = paste0(
      dataset_name, "_", dplyr::row_number()
    ),
    age = (-as.numeric(time_id) * 100) + 2000
  )
