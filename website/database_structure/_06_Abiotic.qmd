## Abiotic Variables

The [abiotic data]{.abiotic} in the [VegVault]{.vegvault} database provide essential information on [environmental factors]{.abiotic} affecting vegetation distribution and [traits]{.traits}. Currently, [VegVault]{.vegvault} includes [abiotic data]{.abiotic} from [CHELSA](https://chelsa-climate.org/), [CHELSA-TRACE21](https://chelsa-climate.org/chelsa-trace21k/), and [WoSIS](https://www.isric.org/explore/wosis). [CHELSA]{.abiotic} and [CHELSA-TRACE21]{.abiotic} provide high-resolution [climate data]{.abiotic}, while [WoSIS]{.abiotic} offers detailed [soil information]{.abiotic}.

```{r}
#| label: source config
#| include: FALSE
#| results: 'hide'
#| warning: FALSE
#| message: FALSE

library(here)

here::i_am("website/database_structure/_06_Abiotic.qmd")

# Load configuration
source(
  here::here(
    "R/00_Config_file.R"
  )
)

source(
  here::here(
    "R/03_Visualisation/00_Config_quarto.R"
  )
)

invisible(
  lapply(
    list.files(
      path = here::here("R/Functions"),
      pattern = "*.R",
      recursive = TRUE,
      full.names = TRUE
    ),
    source
  )
)
```

### [Abiotic variables]{.abiotic} (`AbioticVariable`)

As [VegVault]{.vegvault} contains [abiotic variables]{.abiotic} from several [primary sources]{.database}, the `AbioticVariable` table contains descriptions of [abiotic variables]{.abiotic} (`abiotic_variable_name`), their units (`abiotic_variable_unit`), and measurement details (`measure_details`). These data include variables such as [climate]{.abiotic} and [soil conditions]{.abiotic}, which are crucial for understanding the ecological contexts of vegetation dynamics. 

```{r}
#| label: AbioticVariable variables scheme structure
#| tbl-cap: "Column names and types for table `AbioticVariable`."
data_comments_abiotic_variable <-
  tibble::tribble(
    ~column_name, ~description,
    "abiotic_variable_id", "ID of a Abiotic Variable",
    "abiotic_variable_name", "Name of a Abiotic Variable from primary source",
    "abiotic_variable_unit", "Unit of a Abiotic Variable",
    "abiotic_variable_scale", "Scale of a Abiotic Variable",
    "measure_details", "Additional details about Abiotic Variable"
  )

get_db_table_data_types(
  con,
  "AbioticVariable"
) %>%
  dplyr::full_join(
    data_comments_abiotic_variable,
    by = "column_name"
  ) %>%
  plot_table()
```

```{r}
#| label: AbioticVariable scheme structure
include_graphics_absolute_path(
  here::here(
    "Outputs/Figures/website/DB_scheme_visualisation/AbioticVariable.png"
  )
)
```

```{r}
#| label: show abiotic variables
#| tbl-cap: "Table showing abiotic variables."
dplyr::tbl(con, "AbioticVariable") %>%
  dplyr::select(abiotic_variable_name, abiotic_variable_unit, measure_details) %>%
  dplyr::collect() %>%
  rlang::set_names(
    nm = c(
      "Variable name",
      "Variable unit",
      "Source of data"
    )
  ) %>%
  plot_table()
```

### [Abiotic Data]{.abiotic} (`AbioticData`) 

The `AbioticData` table holds the actual values of abiotic variables (the units are the same for each `AbioticVariable`).

```{r}
#| label:  AbioticData variables scheme structure
#| tbl-cap: "Column names and types for table `AbioticData`."
data_comments_abiotic_data <-
  tibble::tribble(
    ~column_name, ~description,
    "sample_id", "ID of a Sample",
    "abiotic_variable_id", "ID of an Abiotic Variable",
    "abiotic_value", "Value of an Abiotic Variable"
  )
```

```{r}
#| label: AbioticData scheme structure
include_graphics_absolute_path(
  here::here(
    "Outputs/Figures/website/DB_scheme_visualisation/AbioticData.png"
  )
)
```


### Gridpoints (`AbioticDataReference`)

Gridpoints are stored in artificially created [Datasets]{.database} and [Samples]{.database}, with one [Dataset]{.database} holding more [Samples]{.database} only if they differ in age. We have estimated the spatial and temporal distance between each [gridpoint]{.abiotic} and other non-[gridpoint]{.abiotic} [Samples]{.database} (`vegetation_plot`, `fossil_pollen_archive`, and `traits`). We store the link between [gridpoint]{.abiotic} and non-[gridpoint]{.abiotic} [Samples]{.database} as well as the spatial and temporal distance. As this results in very large amounts of data, we have discarded any [gridpoint]{.abiotic} [Sample]{.database}, which is not close to 50 km and/or 5000 years to any other non-[gridpoint]{.abiotic} [Samples]{.database} as not relevant for the vegetation dynamics.

```{r}
#| label: AbioticDataReference variables scheme structure
#| tbl-cap: "Column names and types for table `AbioticDataReference`."
data_comments_abiotic_data_reference <-
  tibble::tribble(
    ~column_name, ~description,
    "sample_id", "ID of non-gridpoint Sample",
    "sample_ref_id", "ID of gridpoint Sample",
    "distance_in_km", "Distance among samples expressed in kilometres",
    "distance_in_years", "Distance among samples expressed in years"
  )

get_db_table_data_types(
  con,
  "AbioticDataReference"
) %>%
  dplyr::full_join(
    data_comments_abiotic_data_reference,
    by = "column_name"
  ) %>%
  plot_table()
```

```{r}
#| label: AbioticDataReference scheme structure
include_graphics_absolute_path(
  here::here(
    "Outputs/Figures/website/DB_scheme_visualisation/AbioticDataReference.png"
  )
)
```

Such data structure allows that environmental context is readily available for each vegetation and trait [Sample]{.database}. For each non-[gridpoint]{.abiotic} [Sample]{.database}, users can select the closest spatio-temporally abiotic data or get average from all surrounding [gridpoints]{.abiotic}.

```{r}
#| label: distribution of gridpoints
x_lim <- c(12, 18.9)
y_lim <- c(48.5, 51.1)

x_lim_min <- min(x_lim)
x_lim_max <- max(x_lim)
y_lim_min <- min(y_lim)
y_lim_max <- max(y_lim)

data_grid_coord <-
  dplyr::inner_join(
    dplyr::tbl(con, "Datasets"),
    dplyr::tbl(con, "DatasetTypeID"),
    by = "dataset_type_id"
  ) %>%
  dplyr::filter(dataset_type == "gridpoints") %>%
  dplyr::filter(
    coord_long >= !!rlang::enquo(x_lim_min) &
      coord_long <= !!rlang::enquo(x_lim_max)
  ) %>%
  dplyr::filter(
    coord_lat >= !!rlang::enquo(y_lim_min) &
      coord_lat <= !!rlang::enquo(y_lim_max)
  )

fig_data_grid_coord <-
  data_grid_coord %>%
  dplyr::collect() %>%
  ggplot2::ggplot(
    mapping = ggplot2::aes(
      x = coord_long,
      y = coord_lat
    )
  ) +
  ggplot2::coord_quickmap(
    xlim = x_lim,
    ylim = y_lim
  ) +
  ggplot2::labs(
    title = "Example of gridpoints resolution",
    x = "Longtitude",
    y = "Latitude"
  ) +
  ggplot2::borders(
    fill = NA,
    col = col_blue_light, # [config]
    size = line_size * 10 # [config]
  ) +
  ggplot2::geom_point(
    shape = 15,
    size = point_size / 2,
    alpha = 0.75,
    col = col_green_dark
  )

save_figure_to_render(fig_data_grid_coord)
```

```{r}
#| label: exampe of abitic data - bio1

# Acess the VegVault file
data_example_all <-
  vaultkeepr::open_vault(
    path = path_to_vegvault # [config]
  ) %>%
  # Add the dataset information
  vaultkeepr::get_datasets() %>%
  # Select modern plot data and climate
  vaultkeepr::select_dataset_by_type(
    sel_dataset_type = c(
      "vegetation_plot",
      "gridpoints"
    )
  ) %>%
  # Limit data to Czech Republic
  vaultkeepr::select_dataset_by_geo(
    lat_lim = c(48.5, 51.1),
    long_lim = c(12, 18.9)
  ) %>%
  # Add samples
  vaultkeepr::get_samples() %>%
  # select only modern data
  vaultkeepr::select_samples_by_age(
    age_lim = c(0, 0)
  ) %>%
  # Add abiotic data
  vaultkeepr::get_abiotic_data() %>%
  # Select only Mean Anual Temperature (bio1)
  vaultkeepr::select_abiotic_var_by_name(sel_var_name = "bio1") %>%
  vaultkeepr::extract_data(
    return_raw_data = TRUE,
    verbose = FALSE
  )

data_gridpoints_abiotic <-
  data_example_all %>%
  dplyr::filter(dataset_type == "gridpoints") %>%
  dplyr::select(sample_id, coord_long, coord_lat, sample_id_link) %>%
  dplyr::rename_with(
    .fn = ~ paste0(.x, "_grid")
  )

data_vegetation <-
  data_example_all %>%
  dplyr::filter(dataset_type == "vegetation_plot") %>%
  dplyr::select(sample_id, coord_long, coord_lat) %>%
  dplyr::rename_with(
    .fn = ~ paste0(.x, "_veg")
  )

data_links <-
  data_gridpoints_abiotic %>%
  dplyr::left_join(
    data_vegetation,
    by = c("sample_id_link_grid" = "sample_id_veg")
  )

fig_gridpoint_links_example <-
  ggplot2::ggplot() +
  ggplot2::coord_quickmap(
    xlim = x_lim,
    ylim = y_lim
  ) +
  ggplot2::labs(
    title = stringr::str_wrap("Example of vegetation plot distance to gridpoints", fig_width_def),
    x = "Longtitude",
    y = "Latitude"
    # caption = paste(
    #  "Each square represents the gridpoint.", "\n",
    #  "Each circle represents the vegetation plot.", "\n",
    #  "Each line represents the distance between vegetation plot and the closest gridpoint."
    # )
  ) +
  ggplot2::borders(
    fill = NA,
    col = col_blue_light, # [config]
    size = line_size * 10 # [config]
  ) +
  ggplot2::geom_segment(
    data = data_links,
    mapping = ggplot2::aes(
      x = coord_long_grid,
      y = coord_lat_grid,
      xend = coord_long_veg,
      yend = coord_lat_veg
    ),
    col = col_white,
    linewidth = line_size
  ) +
  ggplot2::geom_point(
    data = data_gridpoints_abiotic,
    mapping = ggplot2::aes(
      x = coord_long_grid,
      y = coord_lat_grid
    ),
    shape = 15,
    alpha = 0.75,
    size = point_size / 2,
    col = col_green_dark
  ) +
  ggplot2::geom_point(
    data = data_vegetation,
    mapping = ggplot2::aes(
      x = coord_long_veg,
      y = coord_lat_veg
    ),
    shape = 20,
    alpha = 0.75,
    size = point_size * 0.75,
    col = col_green_light
  )

save_figure_to_render(fig_gridpoint_links_example)

```

### Abiotic Variable Reference (`AbioticVariableReference`)

Each [Abiotic Variable]{.abiotic} can have a separate [Reference]{.database}, in addition to a [Dataset]{.database} and [Sample]{.database}. 

```{r}
#| label: AbioticVariableReference variables scheme structure
#| tbl-cap: "Column names and types for table `AbioticVariableReference`."
data_comments_abiotic_variable_reference <-
  tibble::tribble(
    ~column_name, ~description,
    "abiotic_variable_id", "ID of an Abiotic Variable",
    "reference_id", "ID of a Reference"
  )

get_db_table_data_types(
  con,
  table_name = "AbioticVariableReference"
) %>%
  dplyr::full_join(
    data_comments_abiotic_variable_reference,
    by = "column_name"
  ) %>%
  plot_table()
```

<br>