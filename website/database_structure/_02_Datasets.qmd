...
The `Datasets` table represents the main organizational structure in [VegVault]{.vegvault}, serving as the keystone for managing and organizing all data. Each [Dataset]{.database} contains one row with a unique [Dataset ID]{.database} (`dataset_id`), [Dataset name]{.database} (`dataset_name`), geographic location (`coord_lat`, `coord_long`), [Dataset Type]{.database} (`dataset_type_id`), [Dataset Source]{.database} (`data_source_id`), [Dataset Source Type]{.database} (`dataset_source_type_id`), and [Sampling Method]{.database} (`sampling_method_id`).
...
#### [Dataset Types]{.database} (`DatasetTypeID`)

The [Dataset Type]{.database} defines the most basic classification of each [Dataset]{.database}, ensuring that the vast amount of data is categorized systematically. Currently, [VegVault]{.vegvault} contains the following types of [Datasets]{.database}:

-	`vegetation_plot`: This type includes [contemporary vegetation plot data]{.neo-vegetation}, capturing contemporary vegetation characteristics and distributions.
-	`fossil_pollen_archive`: This type encompasses [past vegetation plot data]{.paleo} derived from [fossil pollen records]{.paleo}, providing insights into past vegetation and climate dynamics.
-	`traits`: This type contains [functional trait data]{.traits}, detailing specific characteristics of plant species that influence their ecological roles.
-	`gridpoints`: This type holds artificially created [Datasets]{.database} to manage [abiotic data]{.abiotic}, here [climate]{.abiotic} and [soil information]{.abiotic} (a [Dataset Type]{.database} created to hold [abiotic data]{.abiotic}, see details in the [Database Assemly](/website/database_assembly.qmd#iii-creation-of-gridpoints-for-abiotic-data)).
...
#### [Dataset Source-Types]{.database} (`DatasetSourceTypeID`)

[VegVault]{.vegvault} maintains detailed information about the [primary data source]{.database}, thereby enhancing the findability and referencing of [primary data sources]{.database}. Each [Dataset]{.database} is derived from a specific [Source-Type]{.database} that provides detailed information on the source used to retrieve the original data. The current [Source-Types]{.database} in [VegVault]{.vegvault} include:

- [BIEN]{.neo-vegetation} - [Botanical Information and Ecology Network](https://bien.nceas.ucsb.edu/bien/)
- [sPlotOpen]{.neo-vegetation} - [The open-access version of sPlot](https://idiv-biodiversity.de/en/splot/splotopen.html)
- [TRY]{.traits} - [TRY Plant Trait Database](https://www.try-db.org/TryWeb/Home.php)
- [Neotoma-FOSSILPOL]{.paleo} - [The workflow that aims to process and standardise global palaeoecological pollen data](https://hope-uib-bio.github.io/FOSSILPOL-website/). Note that we specifically state [Neotoma-FOSSILPOL]{.paleo} and not just Neotoma, as [FOSSILPOL]{.paleo} not only provides the [data acquisition]{.reproducibility} but also alters it (e.g., creating new [age-depth models]{.paleo}). It also addresses major challenges in [paleoecological data integration]{.database}, such as [age uncertainty]{.paleo}, by incorporating probabilistic [age-depth models]{.paleo} and their associated [uncertainty matrices]{.paleo}. This enables the propagation of [temporal uncertainty]{.paleo} in subsequent analyses, a critical advancement for robust macroecological studies, previously flagged as a major issue with [paleo-data]{.paleo}.
- [gridpoints]{.abiotic} - artificially created [Datasets]{.database} to hold [abiotic data]{.abiotic}. See [Database Assembly](/website/database_assembly.qmd#iii-creation-of-gridpoints-for-abiotic-data) for more details.
...
data_comments_dataset_source_type <-
  tibble::tribble(
    ...
    "dataset_source_type", "Text description of individual Dataset Source-Type (currently, BIEN, sPlotOpen, TRY, Neotoma-FOSSILPOL, gridpoints)"
  )
...
#### [Dataset Sources]{.database} (`DatasetSourcesID`)

Each individual [Dataset]{.database} from a specific [Dataset]{.database} [Source-Type]{.database} can have information on the source of the data (i.e. sub-database). [VegVault]{.vegvault} v1.0.0 currently includes 706 sources of [Datasets]{.database}, where each dataset can also have one or more direct references to specific data, ensuring that users can accurately cite and verify the sources of their data. This should help to promote better findability of the primary source of data and referencing.
...
Currently, there are `r n_total_sources` sources of [Datasets]{.database}.
...
#### [Sampling Methods]{.database} (`SamplingMethodID`)

Sampling methods vary significantly across the different types of [Datasets]{.database} integrated into [VegVault]{.vegvault}, reflecting the diverse nature of the data collected. Such information is crucial for understanding the context and limitations of each [Dataset Type]{.database}. For [contemporary vegetation plots]{.neo-vegetation}, sampling involves standardised plot inventories and surveys that capture detailed vegetation characteristics across various regions. [Fossil pollen]{.paleo} data are collected from sediment records from numerous different depositional environments representing past vegetation. Therefore, information on sampling methods is only available for both `vegetation_plot` and `fossil_pollen_archive` [Datasets]{.database}, providing metadata that ensures accurate and contextually relevant analyses.
...
#### [Dataset References]{.database} (`DatasetReferences`)

To support robust and transparent scientific research, each [Dataset]{.database} in [VegVault]{.vegvault} can have multiple references at different levels. The [Dataset]{.database} [Source-Type]{.database}, [Dataset Source]{.database}, and [Sampling Method]{.database} can all have their own references, providing detailed provenance and citation information. This multi-level referencing system enhances the traceability and validation of the data. Each [Dataset]{.database} can also have one or more direct references to specific data, further ensuring that users can accurately cite and verify the sources of their data. 
...
We have selected dataset ID: `r as.character(sel_dataset_for_example)`, which is a [fossil pollen archive]{.paleo}. Therefore, it has the reference of the *Dataset Source-Type*: 
