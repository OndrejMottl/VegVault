...
[Samples]{.database} represent the main unit of data in [VegVault]{.vegvault}, serving as the fundamental building blocks for all analyses. There are currently over 13 million [Samples]{.database} in [VegVault]{.vegvault} v1.0.0 (of which ~ 1.6 million are [gridpoints]{.abiotic} of [abiotic data]{.abiotic}, see [Database Assembly](/website/database_assembly.qmd#iii-creation-of-gridpoints-for-abiotic-data)). 

[VegVault]{.vegvault} encompasses both [contemporary]{.neo-vegetation} and [paleo data]{.paleo}, necessitating accurate [age information]{.paleo} for each [Sample]{.database}. [Contemporary]{.neo-vegetation} [Samples]{.database} are assigned an age of `0`, while [Samples]{.database} from [fossil pollen records]{.paleo} are in [calibrated years before the present (cal yr BP)]{.paleo}. The *present* is here specified as `1950 AD`. 
...
### [Sample Structure Overview]{.database} (`Samples`)

The table contains one [Sample]{.database} per row, with each [Sample]{.database} containing: a unique [Sample ID]{.database} (`sample_id`), [Sample name]{.database} (`sample_name`), temporal information about [Sample]{.database} ([age]{.paleo}), sample site (size of the plot if available; `sample_size_id`), and additional information about sample (`sample_details`; this is currently not being used in v1.0.0.). As [VegVault]{.vegvault} encompasses both [contemporary]{.neo-vegetation} and [paleo-data]{.paleo}, accurate [age information]{.paleo} is required for each [Sample]{.database}. 
...
data_comments_samples <-
  tibble::tribble(
    ...
    "age", "Age of sample. Mainly used for `fossil_pollen_archives`, where note the age of a [Sample]{.database} in calibrated years before present. Note that all contemporary [Samples]{.database}, have age of 0.",
...
### [Dataset-Sample]{.database} (`DatasetSample`)

Each [Sample]{.database} is linked to a specific [Dataset]{.database} via the `DatasetSample` table, which ensures that every [Sample]{.database} is correctly associated with its corresponding [Dataset Type]{.database} (whether it is `vegetation_plot`, `fossil_pollen_archive`, `traits`, or `gridpoint`) and other [Dataset]{.database} properties (e.g., geographic location). One [Dataset]{.database} contains several [Samples]{.database} only in a case where they differ in time (`age`). 
...
### [Sample age uncertainty]{.paleo} (`SampleUncertainty`)

Each [Sample]{.database} from the `fossil_pollen_archive` [Dataset]{.database} is also associated with an [uncertainty matrix]{.paleo} generated during the re-estimation of ages using [FOSSILPOL workflow]([https](https://bit.ly/FOSSILPOL)). This matrix provides a range of potential ages derived from [age-depth modelling]{.paleo}, reflecting the inherent uncertainty in dating [paleoecological records]{.paleo}. 
...
Each [Sample]{.database} in [VegVault]{.vegvault} can have specific [References]{.database} in addition to those at the [Dataset]{.database}-level. These individual [Sample References]{.database} provide detailed provenance and citation information, ensuring that users can trace the origin and validation of each data point. Note that a single [Sample]{.database} can have several [References]{.database}. This level of referencing enhances the transparency and reliability of the data, especially when the dataset continues to be updated in the future. 
