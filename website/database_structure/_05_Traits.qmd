...
### [Traits Structure Overview]{.traits} (`Traits`)

The [Traits]{.traits} table contains the list of [functional traits]{.traits} currently contained in [VegVault]{.vegvault}. The table contains one [Trait]{.traits} per row, with each [Trait]{.traits} containing: a unique [Trait ID]{.traits} (`trait_id`), original [Trait name]{.traits} from primary source (`trait_name`), and [Trait Domain]{.traits} (`trait_domain_id`). [Functional traits]{.traits} of vegetation [taxa]{.database} follow the same structure of [Dataset]{.database} and [Samples]{.database} obtained directly from [Dataset]{.database} [Source-Types]{.database}.
...
### [Traits Domain]{.traits} (`TraitsDomain`)

Traits are grouped into [Trait Domains]{.traits} to allow for easier aggregation of [Traits]{.traits} across data sources. As there are differences in trait names across sources of data and individual [Datasets]{.database}, the [VegVault]{.vegvault} database contains [Trait Domain]{.traits} information to group traits together. In total, six [Trait Domains]{.traits} are present: `Stem specific density`, `Leaf nitrogen content per unit mass`, `Diaspore mass`, `Plant height`, `Leaf area`, `Leaf mass per area`, following [Diaz et al. (2016)](https://www.nature.com/articles/nature16489). Yet, it is up to the user to decide how to further aggregate trait values if multiple trait [Samples]{.database} of one [Trait Domain]{.traits} are available for the same environmental or taxonomic entity.
...
#| tbl-cap: "Overview of Trait Domains and their associated Traits"
...
### [Traits value]{.traits} (`TraitsValue`)

In general, data of functional traits of vegetation taxa follow the same structure of the [Dataset]{.database} and [Samples]{.database} obtained directly from the [Dataset Source-Types]{.database}. Therefore, `TraitsValue` table contains not only the actual measured value of [Trait]{.traits} observation but also information about linking information across [Datasets]{.database}, [Samples]{.database}, and [Taxa]{.database}. This comprehensive linkage ensures that each Trait value is accurately associated with its relevant ecological, environmental and taxonomic context.
...
### [Traits Reference]{.traits} (`TraitsReference`)

To ensure clarity and reproducibility, each [Trait]{.traits} in [VegVault]{.vegvault} can have additional [References]{.database} beyond the general [Dataset]{.database} and [Sample]{.database} [References]{.database}. These [Trait]{.traits}-specific [References]{.database} provide detailed provenance and citation information, supporting rigorous scientific research and enabling users to trace the origins and validation of each trait value. 
