...
The database consists of 31 interconnected tables with 87 fields (variables). For detailed technical information, see our [Database Structure](database_structure.qmd) documentation.
...
::: {.callout-tip}
**Getting Started**: New to [VegVault]{.text-background-black .text-color-white .text-bold}? Check our [Database Access](database_access.qmd) guide for download instructions and our [Usage Examples](database_examples.qmd) for practical applications.
:::
...
fig_vegetation_time <-
  ...
  ggplot2::scale_x_continuous(
    transform = "reverse",
    breaks = seq(20e3, 0, by = -5e3),
    labels = seq(20, 0, by = -5)
  ) +
  ...
  ggplot2::guides(
    fill = ggplot2::guide_legend(override.aes = list(size = point_size))
  ) +
  ggplot2::theme(
    legend.position = "top"
  ) +
...
ggplot2::ggsave(
  here::here("Outputs/Figures/website/DB_overview/fig_db_spatiotemporal_overview.png"),
  plot = fig_vegetation,
  width = 10,
  height = 4,
...
knitr::include_graphics(here::here("Outputs/Figures/website/DB_overview/fig_db_spatiotemporal_overview.png"))
```

## Related Resources

- **[Database Structure](database_structure.qmd)** - Technical database schema
- **[Database Assembly](database_assembly.qmd)** - Overview of the data assembly process
- **[Database Access](database_access.qmd)** - Download and setup instructions
- **[Usage Examples](database_examples.qmd)** - Practical applications
- **[Materials & Outputs](materials.qmd)** - Publications and resources