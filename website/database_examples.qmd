...
# [VegVault]{.text-background-black .text-color-white .text-bold} Usage Examples

## Introduction to {vaultkeepr}

:::: {.columns}
::: {.column width="75%"}
To make [VegVault]{.text-background-black .text-color-white .text-bold} as accessible as possible, we developed [{vaultkeepr}](https://bit.ly/vaultkeepr), a comprehensive R package providing intuitive functions for interacting with the [database]{.text-background-brownDark .text-color-white .text-bold}. The package handles database connections, data filtering, and extraction, allowing you to retrieve only project-specific data without loading everything into memory. With >95% code coverage, {vaultkeepr} is a [well-tested]{.text-background-brownLight .text-color-white .text-bold} interface for [VegVault]{.text-background-black .text-color-white .text-bold}.
:::
...
1. **[Download VegVault](/website/database_access.qmd)** database and install {vaultkeepr}
...
The following examples demonstrate [VegVault]{.text-background-black .text-color-white .text-bold}'s capabilities across different research scenarios. Note that these examples focus on **data extraction** rather than analysis—they show how to obtain properly formatted [datasets]{.text-background-brownDark .text-color-white .text-bold} for your research projects.
...
- **[Database Structure](/website/database_structure.qmd)** - Technical database documentation  
